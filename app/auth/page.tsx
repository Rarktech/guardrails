...
import { createBypassSession, saveSession } from "@/lib/zklogin";
...
export default function AuthPage() {
  ...
  const WALLETS = [
    { name: "Sign in with Google", sub: "via zkLogin — no seed phrase", ico: "🔑", tag: "HOT", hot: true },
    { name: "Sui Wallet", sub: "Official Sui browser extension", ico: "🌊", tag: null, hot: false },
    { name: "Phantom", sub: "Multi-chain wallet", ico: "👻", tag: null, hot: false },
    { name: "Suiet", sub: "Open-source Sui wallet", ico: "🔷", tag: null, hot: false },
  ];
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center", maxWidth: 1100, margin: "0 auto" }}>
            ...
            <div className="card" style={{ maxWidth: 460, borderRadius: 28, padding: "32px 28px", boxShadow: "8px 8px 0 rgba(31,26,36,0.9)" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {WALLETS.map(w => (
                  <button key={w.name} className="wallet-btn" onClick={() => connect(w.name)}>
                    <div className="wallet-ico">{w.ico}</div>
                    <div className="wallet-meta" style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                      <span>{w.name}</span>
                      <span className="wallet-sub" style={{ fontSize: 12, fontWeight: 600, color: "var(--ink-soft)" }}>{w.sub}</span>
                    </div>
                    {w.tag && <span className={`wallet-tag ${w.hot ? "hot" : ""}`}>{w.tag}</span>}
                  </button>
                ))}
              </div>
